 ...
 * ============================================================================
 * 
 * Para buscar os dados diretamente e em tempo real da API oficial do TMDB:
 * 1. Crie uma conta gratuita em: https://www.themoviedb.org/signup
 * 2. Obtenha sua chave de API (v3 auth) em: https://www.themoviedb.org/settings/api
 * 3. Cole sua chave no espaço abaixo entre as aspas:
 * 
 * Exemplo: export const TMDB_API_KEY = "3fd2beeb0d7042ba3d02a05d99e";
 * ============================================================================
 */

// >>>>>> INSIRA SUA CHAVE DA API TMDB AQUI: <<<<<<
export const TMDB_API_KEY: string = ""; 
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

// Chave persistida no navegador (permite configurar também pela interface do app)
export const getActiveTmdbKey = (): string => {
  // 1. Chave configurada manualmente na constante acima
  if (TMDB_API_KEY && TMDB_API_KEY.trim() !== "" && TMDB_API_KEY !== "SUA_API_KEY_AQUI") {
    return TMDB_API_KEY.trim();
  }
  
  // 2. Chave salva no LocalStorage (configurada pelo usuário na interface)
...
export const getPosterUrl = (path: string | null | undefined): string => {
  ...
  return `${TMDB_IMAGE_BASE_URL}${TMDB_IMAGE_POSTER_SIZE}${normalizedPath}`;
};
...
export const getBackdropUrl = (path: string | null | undefined): string => {
  ...
  // Garante que o caminho inicie com barra única
  const normalizedPath = clean.startsWith('/') ? clean : `/${clean}`;
  return `${TMDB_IMAGE_BASE_URL}${TMDB_IMAGE_BACKDROP_SIZE}${normalizedPath}`;
};
